...
function pageTwo() {
  ...
  $("#left-side").html(
    "<div><h1>Location & Time Zone</h1><h2>Select Your Country</h2><h4 class='faded'>Enter Your City</h4><h1 class='faded'>Open Weather API</h1></div>"
  );
  $("#right-side").html(
    "<div><div id='top-half-countries'></div><div id='selected-country' onclick='pageThree();'></div><div id='bottom-half-countries'></div></div>"
  );
...
function initRightSideCountriesList() {
  ...
  for (var i = currentSelectedCountry - 9; i < currentSelectedCountry; i++) {
    ...
    document.getElementById("top-half-countries").innerHTML +=
      "<div class='button button-other-country " +
      elementVisible +
      "'>" +
      countries[i] +
      "</div>";
  ...
  document.getElementById("selected-country").innerHTML =
    "<div class='button button-selected button-selected-country'>" +
    countries[currentSelectedCountry] +
    "</div>";

  for (
    var i = currentSelectedCountry + 1;
    i < currentSelectedCountry + 10;
    i++
  ) {
    var elementVisible = "visible";
    if (i > 245) {
...
function pageTwoListener(e) {
  ...
  if (e.keyCode == "38") {
    ...
    // Enter
    $.ajax({
      url: "setcountry/" + currentSelectedCountry,
...
function pageThree() {
  ...
  $("#left-side").html(
    "<div><h1>Location & Time Zone</h1><h2>Enter Your City</h2><h1 class='faded'>Open Weather API</h1></div>"
  );
  $("#right-side").html(
...
function doChange() {
  ...
  for (var i = 0; i < cities[currentSelectedCountry].length; i++) {
    ...
    if (
      results < 5 &&
      inputBoxValue ==
        cities[currentSelectedCountry][i]
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result == "true") {
        ...
        $("#left-side").html(
          "<div><h1>Open Weather API</h1><h2>Enter your Open Weather API Key</h2><p>Please goto <code>openweathermap.org/price</code>,<br /> and sign up for a free account to have an API key emailed to you.</p><p>Want to copy/paste the key?<br> On a phone connected to that same network as your Raspberry Pi,<br> navigate to <code>" +
            deviceInformation["ipaddress"] +
            ":8081/setup/mobile/index.html</code></p></div>"
        );
        $("#right-side").html(
          "<div><div class='autocomplete'><input type='text' name='weather-api-input-box' class='weather-api-input-box button-selected-country button-selected' id='weather-api-input-box'></div><div id='triggerPageFive' onclick='pageFive();'></div></div>"
        );
      }
    },
  });
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result == "true") {
        ...
        $("#right-side").html(
          '<div><div class="button button-full-width button-selected">Let\'s Start</div></div>'
        );
...
function pageFive() {
  $.ajax({
    url:
      "setweatherapikey/" +
      document.getElementById("weather-api-input-box").value,
    success: function (result) {
      console.log(result);
      if (result == "true") {
        $("#left-side").html("<div><h1>All Set!</h1></div>");
